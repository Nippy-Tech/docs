...
1. Inicia sesión en [https://plataforma.nippy.la](https://plataforma.nippy.la)
...
2. **Analytics** tiene doble capa de aislamiento: el prompt del LLM incluye el `businessId` como guía, y el servidor inyecta el filtro server-side **después** de que el LLM genera el pipeline, sobrescribiendo cualquier filtro que el modelo haya puesto.
3. **Roulettes, Supply, Learning, Flows** usan filtros hardcodeados en cada método del servicio (`businessId`, `companyId`, `businesses`) — no pasan por el LLM.
